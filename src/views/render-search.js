export function renderSearchCards(cards, query = '') {
  const normalizedQuery = (query || '').trim().toLowerCase();
  const visibleCards = normalizedQuery ? cards : cards.slice(0, 50);
  ...
  const rows = visibleCards.map((card) => {
    return `
      <tr class="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
        ...
        <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-200 whitespace-nowrap">${escapeHtml(card.grupo)}</div>
  ...
  return `
    <div class="space-y-4">
      ...
      <p class="text-xs text-gray-500 dark:text-gray-400">
        Resultados: <span id="search-count">${cards.length}</span>
        ${normalizedQuery ? String(visibleCards.length) : ''}
      </p>
      ...
      <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            ${visibleCards.length > 0 ? rows : emptyState}
          </tbody>
        </table>
      </div>
    </div>
